/*
	1) Cree una estructura Cuerpo que contenga los siguientes atributos:
		a) int x;
		b) int y;
		c) string caracter;
		d) string color;
		
	2) Con su nueva estructura modifique el arreglo
		int cuerpo[200][2];
	   
	   para poder utilizar un arreglo unidimensional de la estructura Cuerpo
	
	3) Modifique las siguientes funciones
		a) void inicializarArreglo()
		b) void dibujarcuerpo()
		c) void guardarposicion()
		d) void borrarcuerpo()
		e) bool gameover()
		
	   Donde encuentre cuerpo[i][0] debe cambiarlo por cuerpo[i].x
	   Donde encuentre cuerpo[i][1] debe cambiarlo por cuerpo[i].y
	   
	4) Cambie el caracter del cuerpo de la serpiente en void dibujarcuerpo()
	   actualmente se utiliza cout<<"O" para usar cuerpo[i].caracter
	   
	   Inicialice el valor del atributo caracter en void inicializarArreglo()
	   
	5) Agregue un nuevo nivel de dificultad en void dificultad()
